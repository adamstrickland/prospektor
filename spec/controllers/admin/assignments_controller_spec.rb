...
describe Admin::AssignmentsController do
  ...
  describe "if assigning some leads," do
    before :each do
      ...
      @employee.should_receive(:state_or_province).any_number_of_times.and_return(@state)
      
      @user = mock_model(User)
      @user_id = @user.id.to_s
      User.stub!(:find).with(@user_id).and_return(@user)
      @user.stub!(:employee).and_return(@employee)
    ...
    describe "if there are open leads," do
      before :each do
        Lead.should_receive(:open).at_least(:once).and_return(Lead)
      end
      
      it "if a timezone is selected, use that one" do
        Lead.should_receive(:located_in_timezone).with('C').and_return(@leads)
        post :create, :size => @assignment_size, :user_id => @user_id, :time_zone => 'C', :format => 'json'
