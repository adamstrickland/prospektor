          ...
          unless respond_to?(negative_name)
            if !lambda
              if options.include?(:conditions)
                negative_args = args_for_original.except(:conditions)
                negative_args[:conditions] = "NOT (#{ sanitize_sql_for_conditions(options[:conditions]) })"
                named_scope_without_negative_named_scope(negative_name, lambda { 
                  negative_args 
                }, &block)
              end
            else
